...
select unattended in "Activate" "Deactivate" "Skip"; do
    case $unattended in
        Activate ) sudo sed -i 's/"0"/"1"/g' /etc/apt/apt.conf.d/20auto-upgrades 2>&1; echo "Autoupgrade activated"; break;;
        Deactivate ) sudo sed -i 's/"1"/"0"/g' /etc/apt/apt.conf.d/20auto-upgrades 2>&1; echo "Autoupgrade deactivated"; break;;
        Skip) echo "Skipping"; break;;
        *) echo "Skipping"; break;;
    esac
done
